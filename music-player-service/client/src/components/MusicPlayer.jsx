...
class MusicPlayerOnFooter extends React.Component {
  ...
  componentDidMount() {
    $.ajax({
      type: 'GET',
      url: 'http://localhost:4000/getsong/1', //'http://3.15.24.104:4000/getList'
      success: data => {
        this.setState({
          playList: data,
          selected: data.songurl // for all songs, use data[0].songurl
        });
      }
    });
  }
  ...
  timeUpdate() {
    ...
    var current_SecondsFixed = currentSecond.toFixed();
    var current_Time =
        ...
        <Music_Player>
          ...
          {this.state.playList.length ? (
            <Music_Info id="music-info">
              {this.state.playList[this.state.trackNumber].song_title}
              <small> by </small>
